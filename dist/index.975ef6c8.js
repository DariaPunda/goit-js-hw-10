...
inputEl.addEventListener("input", (0, _lodashDebounceDefault.default)(handleInput, DEBOUNCE_DELAY));
function handleInput(e) {
    const inputCountryValue = e.target.value.trim();
    console.log(inputCountryValue);
    (0, _fetchCountries.fetchCountries)(inputCountryValue).then(renderCountryInfo).catch(error);
}
function renderCountryInfo(data) {
    ...
    const countryFullInfo = data.map(({ capital , population , languages  })=>{
        return `<h2 style="font-size:20px; font-weight: 700; padding-left: 40px">Capital:<span 
            style="font-weight: 400; padding-left: 40px">${capital}</span></h2><p style="font-size:18px; 
            font-weight: 700; padding-left: 40px">Population:<span style="font-weight: 400">
            ${population}</span></p><p style="font-size:18px; font-weight: 700; padding-left: 40px">Languages:<span style="font-weight: 400">
            ${Object.values(languages)}</span></p>`;
    }).join("");
    console.log(countryFullInfo);
    if (data.length > 10) return (0, _notiflixDefault.default).Notify.warning("Too many matches found. Please enter a more specific name.");
...
},{"./css/styles.css":"1CY4s","notiflix":"5z0Oc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./fetchCountries":"fTQqv","lodash.debounce":"3JP5n"}],"1CY4s":[function() {},{}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
